...
def get_synced_symlinks(skill_name: str, locations: dict) -> list[str]:
    """Get list of platform IDs that are symlinks pointing to central repo."""
    repo_path = config.SKILL_REPO / skill_name
    synced = []
    ...
    for p_id, info in locations.items():
        if p_id == "repo":
            continue
        if info["is_symlink"] and info["target"] == repo_path.resolve():
            synced.append(p_id)
...
def uninstall_skill_complete(skill_name: str, locations: dict):
    ...
    # First remove all synced symlinks
    for p_id, info in locations.items():
        if p_id == "repo":
            continue  # Handle repo last
    ...
    # Then remove from central repo
    if "repo" in locations:
        repo_info = locations["repo"]
...
def uninstall_skill_selective(skill_name: str, selected_ids: list[str], locations: dict):
...
def main():
    ...
    for p_id, info in locations.items():
        ...
        print(f"   - {info['name']}: {info['path']} [{type_str}{target_info}]{synced_mark}")
    ...
    print("\n🗑️  Uninstall options:")
    print(f"   1. Complete uninstall (central repo + {len(synced_symlinks)} synced platforms)")

    # Only show selective option if there are non-synced locations
    non_synced = [p_id for p_id in locations.keys() if p_id != "repo" and p_id not in synced_symlinks]
    if non_synced:
    ...
    if not choice or choice == "1":
        ...
        print(f"   - Central Repo ({config.SKILL_REPO / skill_name})")
